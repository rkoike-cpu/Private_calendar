...
### エンドポイントの粒度
- 予定本体の更新(`PUT /api/events/{id}`)と、色分けカテゴリの更新(`PUT /api/events/{id}/category`)を**あえて別エンドポイントに分離**した。理由は、前者はGoogleカレンダー本体への書き込み(自分が主催者でない予定では失敗し得る)、後者はこのアプリのDBのみへの書き込み(必ず成功する)という、**信頼性も影響範囲も全く異なる処理**だったため。1つのエンドポイントにまとめてしまうと、「Google側の更新が失敗したらカテゴリの変更も道連れで失われる」という実際に発生した不具合につながった。エンドポイントを分けたことで、「予定の主催者ではないため件名変更はできないが、色分けだけは必ず反映される」という、利用者にとって自然な挙動を実現できた
...
- この判断は元々「同期ボタンを押した時だけ最新化する」というシンプルなUXを実現するために採用したもので、当初から複数ユーザー対応を明確に意図していたわけではない。しかし結果的に、**複数ユーザーに対応しても、DBの負荷が利用者数やその予定数に比例して増えない**という利点を持つ設計になっている。もし予定データをローカルにキャッシュする設計にしていた場合、利用者が増えるほど「人数分の予定データ+同期のたびの差分管理」というデータ量・実装複雑さの両方が比例して増大する問題を抱えていたはずである
- 個人用アプリだからこそ成立する設計というより、**そもそも利用者数のスケールに対して破綻しにくい設計を、個人用の理由で先に採用していた**、という方が正確な捉え方だと考えている
- 一方でトレードオフもある。オフライン時には何も表示できない、過去の予定の傾向を分析するような機能は作りにくい、といった制約は残る

## まとめ

### 設計中に苦労したこと・判断に迷ったこと
- **認証設計の作り直し**: 当初は「Googleにサインインした人がそのままダッシュボードを見られる」という一体型の認証で作っていたが、会社のセキュリティポリシーにより個人のスマホからGoogleサインインができないことが判明し、「Google認証(共有・1回のみ)」と「ダッシュボードへのログイン(合言葉・端末問わず)」を分離する設計に作り直した。これにより認証まわりのテーブル・処理を大きく再設計することになった
- **エンドポイントの粒度の見直し**: 上述の通り、予定の更新とカテゴリの更新を1つのエンドポイントにまとめていたことで、「主催者でない予定は色分けすら反映されない」という不具合が発生した。これをきっかけに、「Google側への書き込みを伴う処理」と「アプリ内のDBのみで完結する処理」を明確に分離するという設計判断に至った
